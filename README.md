...
The PyTorch fraud scorer is done: trained on the transaction data with one-hot merchant categories, 0.74 test AUPRC, exported to ONNX, and wired into `score_fraud`. The CI gate, deployment, and grounding work will be documented here only after each is implemented and tested.
